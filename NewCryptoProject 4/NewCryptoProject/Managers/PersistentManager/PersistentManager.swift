...
final class PersistentManager: PersistentManagerProtocols {
    ...
    func fetchStorageData(completion: @escaping (() -> Void)) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            do {
                ...
                AlertManager.shared.showAlert(with: "Failure!",
                                              title: "Fetch request has failed.",
                                              buttonTitle: "OK") { }
            }
        }
    }
        ...
        for result in results as! [NSManagedObject] {
            if let name = result.value(forKey: "name") as? String,
               ...
               let price = result.value(forKey: "price") as? Float {
                savedList.append(.init(assetId: id,
                                       name: name,
                                       priceUsd: price,
                                       idIcon: nil,
                                       imageUrl: result.value(forKey: "url") as? String))
            }
        }
    }
    ...
    func addToBookMark(_ model: Crypto?, completion: @escaping (() -> Void)) {
        ...
        saveData.setValue(model.priceUsd, forKey: "price")
        saveData.setValue(model.imageUrl, forKey: "url")
        saveData.setValue(model.assetId, forKey: "id")
    ...
    func deleteBookmark(_ model: Crypto?, completion: @escaping (() -> Void)) {
        ...
        do {
            ...
            for data in fetchedResults {
                if let id = data.value(forKey: "id") as? String, let assetId = model.assetId {
                    if id == assetId {
